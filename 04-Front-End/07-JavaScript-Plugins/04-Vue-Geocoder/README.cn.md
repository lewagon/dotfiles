...
可以前往在`index.html`文件里查看，已经有表单的模版了。它应该包含一个 `"text"` 类型的 `input` ，用户可以在其中输入地址，以及一个 `"submit"` 类型的 `input` ，来显示按钮。

完成后，你可以使用 `submit` 事件来捕获用户提交表单的时刻。这时，你会想要使用 `fetch` 触发对MapBox地理编码服务的AJAX查询（可以回到昨天课堂的幻灯片查看）。

**提示： 你应该使用Vue来写JS的代码**

可以回到今天课堂的幻灯片，来查看使用Vue的方式。
...
要使用 `mapbox-gl` 包在 `#map` 中显示地图，首先需要在使用Vue之前导入这个包：
